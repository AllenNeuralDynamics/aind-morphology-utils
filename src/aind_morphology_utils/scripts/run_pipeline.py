...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--slicer-transform', type=str)
...
def main() -> None:
    ...
    if slicer_transform_file is not None:
        slicer_transform = OMEZarrTransform(slicer_transform_file)
    ...
    for swc_file in all_swcs:
        ...
        # save using same directory structure as input folder, make sure all directories exist
        transformed_path = os.path.join(aligned_dir, os.path.relpath(swc_file, neuron_folder))
    ...
    merge_swcs_in_folder(aligned_dir, merged_dir, ignore_list=['unique', 'base'])
